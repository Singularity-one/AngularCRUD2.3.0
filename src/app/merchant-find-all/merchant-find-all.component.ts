...
export class MerchantFindAllComponent implements OnInit {
    ...
    this.merchantService.getUsers().subscribe( data =>{
      ...
      if('0000'=== returnText){
       ...
       for(i=0;i<this.merchants.length;i++){
        ...
        //this.user_photo = this.sanitizer.bypassSecurityTrustResourceUrl('data:image/jpeg;base64,' +window.atob(this.merchants[i]["pic"]));//看解碼是否正確
  ...
  get jsonMerchant(){
    return JSON.stringify(this.newMerchant);
  }
